  ...
  <div class="full-width full-height">
    <div class="pl20x mt20x">
      <q-breadcrumbs separator=">" active-color="whiddte">
        ...
        <q-breadcrumbs-el label="新建商户" class="fw_600 text-panda-text-7 fs14" />
      </q-breadcrumbs>
    </div>
        ...
        <div class="row">
          <div class="col-lg-6 col-md-12">
                ...
                <div class>
                  ...
                  <div class="append-handle-btn-input row">
                    <q-input
                      ...
                      lazy-rules
                      :rules="[
                      val => val !== null && val !== '' || '商户编号不能为空！',
                      val => /^\d{6}$/.test(val) || '商户编号只能为6位数字！',
                    ]"
                    ></q-input>
                  </div>
                </div>
            ...
            <div class="flex ml40x mt10x">
              <div class>
                <div class="text-panda-text-7 mb12x fw_600">国家</div>
                <div class="append-handle-btn-input row">
                  <q-input
                    style=" width:240px;"
                    outlined
              ...
              <div class="ml65x">
                <div class="text-panda-text-7 mb12x fw_600">省市地区</div>
                <div class="append-handle-btn-input row">
                  <q-input
                    style=" width:240px;"
                    outlined
            ...
            <div class="flex ml40x mb10x mt20x">
              ...
              <div class="ml65x" v-show="formData.currency == 2">
                <div class="text-panda-text-7 mb12x fw_600">
                  兑一人民币汇率
                  <span class="panda-text-red">*</span>
                </div>
          ...
          <div class="col-lg-6 col-md-12">
              ...
              <div class="mt20x pl40x">
                ...
                <div class="append-handle-btn-input position-relative">
                  <div
                    class="fs12 position-absolute"
                    style="left: 12px; top: 9px;"
                    v-if="!formData.transferMode"
                  >请选择商户钱包类型</div>
                  <q-select
                    id="select"
                    class="input-height-36px w-240"
                    v-model="formData.transferMode"
            ...
            <div class="row" v-show="formData.transferMode == 1">
              <div class="ml40x">
                <div class="text-panda-text-7 mb12x fw_600">
                  加扣款接口URL
                  <span class="panda-text-red">*</span>
...
export default {
  ...
  methods: {
    ...
    //生成随机6位数字
    init_random_number() {
      this.formData.merchantCode = Math.random()
        .toString()
        .slice(-6);
    },
    ...
    handleSave() {
      ...
      if (
        ...
        this.formData.contact = contact.slice(0, contact.lastIndexOf(","));
        this.formData.phone = phone.slice(0, phone.lastIndexOf(","));
        let filePath = "";
        this.files.map(item => {
          item.file && (filePath += item.file + ";");
        });
        if (!this.formData.contact && !this.formData.phone) {
          this.$message.warn("商务联系人或者联系电话不能为空！");
          return false;
        } else if (!this.formData.levelName) {
        ...
        this.formData.filePath = filePath.slice(0, filePath.lastIndexOf(";"));
    ...
    validate() {
      // this.$nextTick(() => {
      this.$refs.merchantCode.validate();
      this.$refs.merchantName.validate();
      this.$refs.email.validate();
      this.$refs.currency.validate();
      this.formData.currency == 2 && this.$refs.rate.validate();
      this.$refs.transferMode.validate();
      this.formData.transferMode == 1 && this.$refs.url.validate();
      this.formData.transferMode == 1 && this.$refs.callbackUrl.validate();
      // });
    },
    ...
    handle_function(item) {
      ...
      this.formData.levelName = item.levelName;
      ...
      this.formData.vipAmount = item.vipAmount;
      this.formData.vipPaymentCycle = item.vipPaymentCycle;
      this.formData.techniqueAmount = item.techniqueAmount;
      this.formData.techniquePaymentCycle = item.techniquePaymentCycle;
    }
  },
  watch: {
    tab(v) {
    ...
    "formData.level"(val) {
      // console.log(val)
      api_base
        .get_manage_merchantLevel_getMerchantLevel({ level: val })
        .then(res => {
          // console.log(res.data);
          let { code, data } = res.data;
          if (code == "0000000") {
            this.currentLevelObj = data;
          }
        });
    }
  }
