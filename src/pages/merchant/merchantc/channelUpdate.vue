  ...
  <div class="full-width full-height">
    <div class="pl20x mt20x">
      <q-breadcrumbs separator=">" active-color="whiddte">
        ...
        <q-breadcrumbs-el label="编辑渠道商户" class="fw_600 text-panda-text-7 fs14" />
      </q-breadcrumbs>
    </div>
      ...
      <a-form id="components-form-edit" class="edit" :form="form" @submit="handleSubmit">
        <div class="row">
                ...
                <div class="flex mb20x">
                  <div class>
                    ...
                    <div class="append-handle-btn-input row">
                      ...
                      >{{$route.query.agentLevel === "1"? '渠道商户': '暂定商户'}}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="col-lg-6 col-md-12">
            ...
            <div class="row mt20x">
              <div class="col-lg-3 col-md-2">
                <div class="pl40x">
                  ...
                  <div class="append-handle-btn-input w-180">
                    <a-form-item>
                      <a-select
                        autocomplete
                        placeholder="请选择授予直营权限"
                        size="large"
                        @change="handleDirectSaleChange"
                        v-decorator="[
                        'directSale',
                        { rules: [{ required: true, message: '授予直营商户权限不能为空！' }] },
                      ]"
                      >
                        <a-select-option
                          :value="item.value"
                          v-for="(item, index) in directSaleList"
                          :key="index"
              ...
              <div class="col-6 ml30x">
                <div class="pl40x">
                  ...
                  <div class="append-handle-btn-input position-relative">
                    <a-form-item>
                      <a-radio-group
                        @change="handleChildConnectModeChange"
                        v-decorator="[
                        'childConnectMode',
                        { rules: [{ required: true, message: '二级商户对接模式不能为空！' }] },
                      ]"
                      >
                        <a-radio v-for="(item, index) in childConnectModeList" :key="index" :value="item.value">{{item.label}}</a-radio>
                      </a-radio-group>
                    </a-form-item>
                  </div>
                </div>
            ...
            <div class="row" v-if="detailObj.directSale == 1 || detailObj.childConnectMode == 2">
              <div class="pl40x">
                <div class="text-panda-text-7 mb10x fw_600">
                  商户钱包类型
                  <span class="panda-text-red">*</span>
...
export default {
  ...
  methods: {
    ...
    async initMerchantDetail() {
      ...
      if (code == "0000000") {
        ...
        let initValues = {
          ...
          childMaxAmount: this.detailObj.childMaxAmount,
          ...
          directSale: this.detailObj.directSale, //授予直营商户权限
          childConnectMode: this.detailObj.childConnectMode, //二级商户对接模式
        ...
        if(this.detailObj.directSale !== 1 && this.detailObj.childConnectMode !== 2){
          delete initValues.transferMode;
        }
    ...
    // 授予直营商户权限
    handleDirectSaleChange(val){
      this.detailObj.directSale = val;
      if(this.detailObj.directSale === 2 && this.detailObj.childConnectMode === 1){
        this.detailObj.transferMode = 2;
      }
    },
    // 钱包类型
    ...
    // 二级商户对接模式
    handleChildConnectModeChange(e){
      this.detailObj.childConnectMode = e.target.value;
      if(this.detailObj.directSale == 2 && e.target.value == 1){
        this.detailObj.transferMode = 2;
      }
    },
