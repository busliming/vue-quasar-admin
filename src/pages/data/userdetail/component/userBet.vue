  ...
  <div class="full-width full-height position-relative">
    <div class="pt10x fs14 bg-panda-bg-6 line-height-30px">
      ...
      <a-table
        ...
        :scroll="{ x:1600, y: !is_expand? scrollHeight - 186 - 48 + 52: scrollHeight - 186 - 48 }"
        :pagination="false"
...
export default {
    ...
    isDay(val){
      ...
      if(val){
        ...
        if(this.is_date_change){
          // 手动选中了查询时间
          // console.log()
          this.searchForm.startTime = moment(startTime).format('YYYY-MM-DD')+ ' 00:00:00';
          this.searchForm.endTime = moment(endTime).format('YYYY-MM-DD')+ ' 23:59:59';
        }else{
          // 默认查询时间
          this.searchForm.startTime = moment(this.start_time).format('YYYY-MM-DD') + ' 00:00:00';
          this.searchForm.endTime = this.end_time + ' 23:59:59';
        }     
      }else{
        // 账务日 是从今天的12:00:00 到 明天的11:59:59
        if(this.is_date_change){
          this.searchForm.startTime = moment(startTime).format('YYYY-MM-DD')+ ' 12:00:00';
          this.searchForm.endTime = moment(endTime).format('YYYY-MM-DD')+ ' 11:59:59';
        }else{
          // 默认查询时间
          this.searchForm.startTime = moment(this.start_time).format('YYYY-MM-DD') + ' 12:00:00';
          this.searchForm.endTime = this.end_time + ' 11:59:59';
        }
      }
    },
  ...
  methods: {
    ...
    init_assignment(){
      let { timeZone, dateType, filter, userId } = this.$route.query;
      this.searchForm.userId = userId;
      this.searchForm.orderStatusList= filter == 3?[1]: [];
      this.pagination.filter = filter;
      this.isDay = timeZone === 'UTC8'? true: false;
      let {startDate, endDate, startDate1, endDate1} = this.$q.sessionStorage.getItem('record');
      this.start_time = dateType === 'day'? startDate: moment(startDate1).format('YYYY-MM-DD');
      this.end_time =dateType === 'day'? endDate: moment(endDate1).add(1, 'months').subtract(1, 'days').format('YYYY-MM-DD');
      console.log(this.start_time,this.end_time,moment(this.end_time).add(1, 'months').format('YYYY-MM-DD'),'start_time');
    },
    handle_expand(){
